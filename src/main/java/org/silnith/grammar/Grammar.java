...
public class Grammar<T extends TerminalSymbol, N extends NonTerminalSymbol> {
    ...
    public static class DefaultMapFactory<K> implements MapFactory<K> {

        @Override
        public <V> Map<K, V> getNewMap() {
            return new HashMap<>();
        }
    ...
    protected class NullableSetComputer implements Callable<Set<N>> {
        
        private final N nonTerminalSymbol;
        
        private final Production production;
    
        public NullableSetComputer(final N nonTerminalSymbol, final Production production) {
            super();
        ...
        @Override
        public Set<N> call() throws Exception {
    ...
    protected class FirstSetComputer implements Callable<Map<N, Set<T>>> {
        
        private final N nonTerminalSymbol;
        
        private final Production production;
    
        public FirstSetComputer(final N nonTerminalSymbol, final Production production) {
            super();
            this.nonTerminalSymbol = nonTerminalSymbol;
            this.production = production;
        }
    ...
    private class FollowSetComputer implements Callable<Map<Symbol, Set<T>>> {
        ...
        public FollowSetComputer(final N nonTerminalSymbol, final Production production) {
            super();
            this.nonTerminalSymbol = nonTerminalSymbol;
            this.production = production;
        }
        ...
        public Map<Symbol, Set<T>> call() throws Exception {
            ...
            final Set<T> nonTerminalFollowSet = getFollowSetForNonTerminal(nonTerminalSymbol);
            ...
            while (forwardIter.hasNext()) {
                ...
                while (innerIter.hasNext()) {
                    ...
                    additionsForSymbol.addAll(getFirstSetForSymbol(endSymbol));
    ...
    private final SetFactory<N> nonTerminalSetFactory;
    
    private final MapFactory<T> terminalMapFactory;
    
    private final MapFactory<N> nonTerminalMapFactory;
     ...
     */
    private final Set<N> nonTerminalSymbols;
    
    /**
     * The mapping of non-terminal symbols to productions.
     */
    private final Map<N, Set<Production>> productions;
    
    /**
     * The nullable non-terminal symbols.
     */
    private final Set<N> nullable;
    
    /**
     * The first set for each non-terminal symbol.
     * <p>
     * The first set for each terminal symbol is the symbol itself.
     */
    private final Map<N, Set<T>> nonTerminalFirstSet;
    
    /**
     * The follow set for each terminal symbol.
     */
    private final Map<T, Set<T>> terminalFollowSet;
    /**
     * The follow set for each non-terminal symbol.
     */
    private final Map<N, Set<T>> nonTerminalFollowSet;
    ...
    public Grammar(final SetFactory<T> terminalSetFactory) {
        ...
        this.nonTerminalSetFactory = new DefaultSetFactory<>();
        this.terminalMapFactory = new DefaultMapFactory<>();
        this.nonTerminalMapFactory = new DefaultMapFactory<>();
        this.lexicon = this.terminalSetFactory.getNewSet();
        this.nonTerminalSymbols = this.nonTerminalSetFactory.getNewSet();
        this.productions = this.nonTerminalMapFactory.getNewMap();
        this.nullable = this.nonTerminalSetFactory.getNewSet();
        this.nonTerminalFirstSet = this.nonTerminalMapFactory.getNewMap();
        this.terminalFollowSet = this.terminalMapFactory.getNewMap();
        this.nonTerminalFollowSet = this.nonTerminalMapFactory.getNewMap();
    ...
    public Set<T> getLexicon() {
        return terminalSetFactory.getNewSet(lexicon);
    }
     ...
     */
    public Set<N> getNullableSet() {
        return nonTerminalSetFactory.getNewSet(nullable);
    }
    ...
    public Map<Symbol, Set<T>> getFirstSet() {
        final Map<Symbol, Set<T>> first = new HashMap<>();
        first.putAll(nonTerminalFirstSet);
    ...
    public Map<Symbol, Set<T>> getFollowSet() {
        final Map<Symbol, Set<T>> follow = new HashMap<>();
        follow.putAll(terminalFollowSet);
        follow.putAll(nonTerminalFollowSet);
        return follow;
    }
    ...
    public void clear() {
        ...
        nullable.clear();
        nonTerminalFirstSet.clear();
        terminalFollowSet.clear();
        nonTerminalFollowSet.clear();
     ...
     */
    public void addProduction(final N nonTerminalSymbol, final ProductionHandler productionHandler,
    ...
    private void addSymbols(final Symbol... symbols) {
        for (final Symbol symbol : symbols) {
            if (symbol instanceof TerminalSymbol) {
                ...
                final N nonTerminalSymbol = (N) symbol;
                nonTerminalSymbols.add(nonTerminalSymbol);
                if ( !productions.containsKey(nonTerminalSymbol)) {
    ...
    private void computeNullable() {
        ...
        do {
            changed = false;
            for (final Map.Entry<N, Set<Production>> entry : productions.entrySet()) {
                final N nonTerminal = entry.getKey();
    ...
    private void computeFirst() {
        nonTerminalFirstSet.clear();
        for (final N nonTerminalSymbol : nonTerminalSymbols) {
            nonTerminalFirstSet.put(nonTerminalSymbol, terminalSetFactory.getNewSet());
        }
        ...
        do {
            changed = false;
            for (final Map.Entry<N, Set<Production>> entry : productions.entrySet()) {
                final N leftHandSide = entry.getKey();
                final Set<Production> productions = entry.getValue();
                
                final Set<T> firstSetForLeftHandSide = getFirstSetForNonTerminal(leftHandSide);
    ...
    private Set<T> getFirstSetForSymbol(final Symbol symbol) {
        if (symbol instanceof TerminalSymbol) {
            return getFirstSetForTerminal((T) symbol);
        } else if (symbol instanceof NonTerminalSymbol) {
            return getFirstSetForNonTerminal((N) symbol);
        } else {
            throw new IllegalArgumentException();
        }
    }
    
    private Set<T> getFirstSetForTerminal(final T symbol) {
        return Collections.singleton(symbol);
    }
    
    private Set<T> getFirstSetForNonTerminal(final N symbol) {
        return nonTerminalFirstSet.get(symbol);
    }
    
    private Set<T> getFollowSetForSymbol(final Symbol symbol) {
        if (symbol instanceof TerminalSymbol) {
            return getFollowSetForTerminal((T) symbol);
        } else if (symbol instanceof NonTerminalSymbol) {
            return getFollowSetForNonTerminal((N) symbol);
        } else {
            throw new IllegalArgumentException();
        }
    }
    
    private Set<T> getFollowSetForTerminal(final T symbol) {
        return terminalFollowSet.get(symbol);
    }
    
    private Set<T> getFollowSetForNonTerminal(final N symbol) {
        return nonTerminalFollowSet.get(symbol);
    }
    ...
    private void computeFollow() {
        ...
        nonTerminalFollowSet.clear();
        for (final T terminalSymbol : lexicon) {
            terminalFollowSet.put(terminalSymbol, terminalSetFactory.getNewSet());
        }
        for (final N nonTerminalSymbol : nonTerminalSymbols) {
            nonTerminalFollowSet.put(nonTerminalSymbol, terminalSetFactory.getNewSet());
        }
        ...
        do {
            ...
            for (final Map.Entry<N, Set<Production>> entry : productions.entrySet()) {
                final N leftHandSide = entry.getKey();
                final Set<T> followSetForLeftHandSide = getFollowSetForNonTerminal(leftHandSide);
                final Set<Production> productions = entry.getValue();
                ...
                for (final Production production : productions) {
                    ...
                    while (revIter.hasPrevious()) {
                        final Symbol symbol = revIter.previous();
                        final Set<T> followSetForSymbolInProduction = getFollowSetForSymbol(symbol);
                    ...
                    while (forwardIter.hasNext()) {
                        final int startIndex = forwardIter.nextIndex();
                        final Symbol startSymbol = forwardIter.next();
                        final Set<T> followSetForRangeStart = getFollowSetForSymbol(startSymbol);
                        final ListIterator<Symbol> innerIter = productionSymbols.listIterator(startIndex + 1);
                        while (innerIter.hasNext()) {
                            final Symbol endSymbol = innerIter.next();
                            final Set<T> firstSetForRangeEnd = getFirstSetForSymbol(endSymbol);
    ...
    private boolean expandFirstSetByProduction(final Set<T> firstSet, final List<Symbol> symbols) {
        ...
        for (final Symbol symbol : symbols) {
            final Set<T> firstSetForSymbolInProduction = getFirstSetForSymbol(symbol);
    ...
    private ParserState<T> calculateClosure(final Collection<LookaheadItem<T>> items) {
        ...
        do {
            ...
            for (final Map.Entry<Item, Set<T>> entry : itemLookaheadMap.entrySet()) {
                ...
                final N nextNonTerminalSymbol = (N) nextSymbol;
    ...
    private Set<LookaheadItem<T>> createInitialItem(final N startSymbol, final T endOfFileSymbol) {
        final Set<T> endOfFileSet = terminalSetFactory.getNewSet(Collections.singleton(endOfFileSymbol));
        
        addProduction((N) START, new IdentityProductionHandler(), startSymbol, endOfFileSymbol);
     ...
     */
    public Parser<T> createParser(final N startSymbol, final T endOfFileSymbol) {
    ...
    private void threadedComputeNullable(final ExecutorService executorService) throws InterruptedException, ExecutionException {
        ...
        final List<Callable<Set<N>>> tasks = new ArrayList<>();
        boolean changed;
        do {
            tasks.clear();
            
            for (final Map.Entry<N, Set<Production>> entry : productions.entrySet()) {
                final N nonTerminalSymbol = entry.getKey();
            ...
            final List<Future<Set<N>>> futures = executorService.invokeAll(tasks);
    
            changed = false;
            
            for (final Future<Set<N>> future : futures) {
                final Set<N> set = future.get();
    ...
    private void threadedComputeFirstSet(final ExecutorService executorService) throws InterruptedException, ExecutionException {
        ...
        do {
            ...
            final List<Future<Map<N, Set<T>>>> futures = executorService.invokeAll(tasks);
            
            changed = false;
            
            for (final Future<Map<N, Set<T>>> future : futures) {
                final Map<N, Set<T>> firstSetAdditions = future.get();
                
                for (final Map.Entry<N, Set<T>> entry : firstSetAdditions.entrySet()) {
                    final N nonTerminalSymbol = entry.getKey();
                    final Set<T> firstSet = entry.getValue();
                    
                    final boolean b = getFirstSetForNonTerminal(nonTerminalSymbol).addAll(firstSet);
        ...
        do {
            ...
            for (final Future<Map<Symbol, Set<T>>> future : futures) {
                ...
                for (final Map.Entry<Symbol, Set<T>> entry : followSetAdditions.entrySet()) {
                    ...
                    final boolean b = getFollowSetForSymbol(symbol).addAll(followSet);
     ...
     */
    public Parser<T> threadedCreateParser(final N startSymbol, final T endOfFileSymbol, final ExecutorService executorService) throws InterruptedException, ExecutionException {
        final long startTime = System.currentTimeMillis();
